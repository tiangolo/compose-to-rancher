    ...
    for service, service_options in compose_v2_data['services'].items():
        ...
        for option in service_options:
            if option == 'depends_on':
                _add_or_merge(new_compose[service], 'links', service_options['depends_on'])

            if option not in EXCLUDED_OPTIONS:
                _add_or_merge(new_compose[service], option, service_options[option])
...
def _add_or_merge(dictionary, key, values):
    """
